...
- Release train helpers validate `skenion.release-train` `0.1.0` manifests,
  require exact SDK/Contracts train versions, and report lockstep mismatch
  diagnostics for Runtime, Studio, Examples, and Manual metadata.
...
validate through
`@skenion/contracts` 0.39.0.

## Release Train Manifest Helpers

SDK tooling can consume the train manifest emitted by Contracts without making
the SDK a second schema authority. The helper uses the public
`@skenion/contracts` `validateReleaseTrainManifestV01` export, then adds
SDK-specific exact-version diagnostics:

```ts
import { readReleaseTrainManifest } from "@skenion/sdk";

const manifest = readReleaseTrainManifest(releaseTrainJson, {
...
The version options are intended for conductor or publish workflow checks after
Release Please has applied the train `release-as` version. Broad semver ranges
may be reported as diagnostics when release tooling supplies them, but this
feature helper does not make the PR package manifest the release authority.
